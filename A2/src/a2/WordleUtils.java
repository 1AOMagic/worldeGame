...
public class WordleUtils {
	...
	public static ArrayList<Boolean> isGreen(String guess, String answer) {
		if (guess.length() != answer.length()) {
			throw new IllegalArgumentException("guess and answer are different lengths");
		}
	...
	public static ArrayList<Boolean> isYellow(String guess, String answer) {
		...
		for (int i = 0; i < guess.length(); i++) {
			if (answer.indexOf(guess.charAt(i)) != -1) {
				if (answer.indexOf(guess.charAt(i)) != i && isGreen(guess, answer).get(answer.indexOf(guess.charAt(i))) == false) {
					t.add(i, true);
				}
				else if (answer.indexOf(guess.charAt(i)) != answer.lastIndexOf(guess.charAt(i)) && isGreen(guess, answer).get(answer.lastIndexOf(guess.charAt(i))) == false){
					t.add(i,  true);
				}
				else {
					t.add(i, false);
	...
	public static ArrayList<WordleColor> getColors(String guess, String answer) {
		...
		for (int i = 0; i < guess.length(); i++) {
			if (isGreen(guess, answer).get(i) == true) {
				t.add(g);
			}
			else if (isYellow(guess, answer).get(i) == true) {
				t.add(y);
			}
			else {
				t.add(gray);
